/*
	...
	Copyright (c) 2002-2013 Un4seen Developments Ltd.
*/
...
{
	static float *buf=0; // buffer for input data
	static DWORD buflen=0;
	if (buflen<length) buf=realloc(buf,buflen=length);
	if (isinput) {
...
static BOOL Initialize()
{
	...
	// start it using default buffer size and only 1 thread (input and output channels must be processed in series)
	if (!BASS_ASIO_Start(0,1)) {
		BASS_ASIO_Free();
		BASS_Free();
