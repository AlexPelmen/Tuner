...
void main(int argc, char **argv)
{
	...
	BASS_Init(0,48000,0,0,NULL); // initialize "no sound" BASS device
	...
	for (a=1;a<ci.chans;a++)
		BASS_ASIO_ChannelJoin(0,a,0); // and join the next channels to it
	if (ci.chans==1) BASS_ASIO_ChannelEnableMirror(1,0,0); // mirror mono channel to form stereo output
	BASS_ASIO_ChannelSetFormat(0,0,ci.flags&BASS_DSD_RAW?BASS_ASIO_FORMAT_DSD_MSB:BASS_ASIO_FORMAT_FLOAT); // set the data format (DSD with MSB 1st, or float in DSP-over-PCM mode)
