...
INT_PTR CALLBACK dialogproc(HWND h,UINT m,WPARAM w,LPARAM l)
{
	switch (m) {
		...
		case WM_INITDIALOG:
			win=h;
			{ // initialize ASIO (1st available device)
				int dev;
				for (dev=0;!BASS_ASIO_Init(dev,0);dev++) {
					if (BASS_ASIO_ErrorGetCode()==BASS_ERROR_DEVICE) { // no more devices to try
			...
			{ // get list of inputs (assuming channels are all ordered in left/right pairs)
				...
				SetTimer(h,0,50,0); // timer to update the display
				return 1;
			}
			break;
